    ...
    public class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            if (!this.IsListening("127.0.0.1", 2050))
            {
                ...
                System.Threading.Thread.Sleep(
                    (int)System.TimeSpan.FromSeconds(2).TotalMilliseconds);
                Process.Start("C:/Users/Admin/Desktop/wServer.bat");
        ...
        private void ErrExit()
        {
            ...
            }//MySql.Data.MySqlClient.MySqlPooledConnection()
            IntPtr intPtr = Form1.FindWindow(null, "all pooled connections were");
            if (hWnd.ToString() != "0")
            {
                Form1.SendMessage(hWnd, 16u, 0, 0);
                this.listBox1.Items.Add("[" + Convert.ToString(DateTime.Now) + "] Killed MySQL");
                Process[] processesByName = Process.GetProcessesByName("mysqld");
                if (processesByName.GetLength(0) > 0)
                {
                    System.Threading.Thread.Sleep(
                        (int)System.TimeSpan.FromSeconds(2).TotalMilliseconds);
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.IsListening("127.0.0.1", 2050))
                {
                    //Process.Start("C:/Users/Admin/Desktop/wServer.bat");
                    System.Threading.Thread.Sleep(
                        (int)System.TimeSpan.FromSeconds(10800).TotalMilliseconds);
                    processes.Kill();
                }
                if (this.IsListening("127.0.0.1", 8889))
                {
                    //Process.Start("C:/Users/Admin/Desktop/server.bat");
                    System.Threading.Thread.Sleep(
                        (int)System.TimeSpan.FromSeconds(10800).TotalMilliseconds);
                    process.Kill();
                    Process[] processesByName = Process.GetProcessesByName("wServer");
