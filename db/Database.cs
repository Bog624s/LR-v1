    ...
    public partial class Database : IDisposable
    {
        ...
        public Database()
        {
            /* _con = new MySqlConnection(
                       "Server=127.0.0.1;Database=rotmg;uid=root;password=;Connection Timeout=999999;MinimumPoolSize=1;maximumpoolsize=9999;"
                       ); //Connection Timeout=999999;MinimumPoolSize=0;maximumpoolsize=100;
              _con.Open();*/
            // _con.Open();
            //   using (_con = new MySqlConnection("Server=127.0.0.1;Database=rotmg;uid=root;password=;convert zero datetime=True; Connection Timeout=4;"))
            //   {
            //    int ipool = 1;
            //    while (ipool == 1)
            //    {
            ...
            try
            {
                _con = new MySqlConnection("Server=localhost;Database=rotmg;uid=;password=;convert zero datetime=True; Connection Timeout=4;");
               // _con.ClearAllPoolsAsync();
                _con.Open();
            }
            catch (Exception e)
            {
...
AND characters.charId=death.chrId;";
        ...
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText = "SELECT id, name, rank, namechosen, verified, guild, guildRank, guildFame, banned, locked, ignored, tag, bonuses FROM accounts WHERE uuid=@uuid AND password=SHA1(@password);";
        ...
        public Account Register(string uuid, string password, bool isGuest)
        {
            ...
            cmd = CreateQuery();
            cmd.CommandText =
                "INSERT INTO accounts(uuid, password, name, rank, namechosen, verified, guild, guildRank, guildFame, vaultCount, maxCharSlot, regTime, guest, banned, locked, ignored, bonuses) VALUES(@uuid, SHA1(@password), @name, 0, 1, 1, 0, 0, 0, 1, 2, @regTime, @guest, 0, @empty, @empty, @empty);";
            cmd.Parameters.AddWithValue("@uuid", uuid);
            cmd.Parameters.AddWithValue("@password", password);
...
SELECT credits FROM stats WHERE accId=@accId;";
        ...
        public int UpdateCharSlot(Account acc, int amount)
        {
            MySqlCommand cmd = CreateQuery();
            if (amount > 0)
            {
                cmd.CommandText = "UPDATE accounts SET maxCharSlot = (maxCharSlot + @amount) WHERE accId=@accId;";
                /*
                    update rotmg.accounts set
	                    maxCharSlot=(maxCharSlot+50)
                        where name="omgsickness";
                */
                cmd.Parameters.AddWithValue("@accId", acc.AccountId);
                cmd.Parameters.AddWithValue("@amount", amount);
                cmd.ExecuteNonQuery();
        ...
        public int UpdateVIP(Account acc, int rank)
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText = "UPDATE accounts SET rank = @rank  WHERE accId=@accId;";
            cmd.Parameters.AddWithValue("@accId", acc.AccountId);
        ...
        public int UpdatePetGift(Account acc, int pettype)
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText = "UPDATE accounts SET bonuses = '0x@pet' WHERE accId=@accId;";
            cmd.Parameters.AddWithValue("@accId", acc.AccountId);
            cmd.Parameters.AddWithValue("@pet", pettype);
            cmd.ExecuteNonQuery();
            cmd = CreateQuery();
            return (int)cmd.ExecuteScalar();
        }
...
SELECT fame FROM stats WHERE accId=@accId;";
        ...
        public int MarketPlace(Account acc, int amount)
        {
            MySqlCommand cmd = CreateQuery();
            if (amount > 0)
            {
                cmd.CommandText = "INSERT INTO marketplace UPDATE stats SET totalFame = totalFame + @amount WHERE accId=@accId;";
                cmd.CommandText = @"INSERT INTO marketplace(accId, type, price, item) VALUES(@accId, '-1, -1, -1, -1, -1, -1, -1, -1');
