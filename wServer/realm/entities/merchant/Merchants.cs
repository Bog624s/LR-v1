...
namespace wServer.realm.entities
{
    public class Merchants : SellableObject
    {
        ...
        protected override void ExportStats(IDictionary<StatsType, object> stats)
        {
            ...
            {
                int[] thelist;
                if (Owner.Map[(int)X, (int)Y].Region == TileRegion.Store_1 && Owner.Name == "The Wineller")
                    thelist = store1List;
                    ...
                    thelist = store9List;
                else if (Owner.Name == "Shopping" && !custom)
    ...
    public class Merchants : SellableObject
    {
        ...
        protected override void ExportStats(IDictionary<StatsType, object> stats)
        {
            if (trueMType == -1)
            {
                ...
                theListL.Shuffle();
                trueMType = theListL[0];
                foreach (var t1 in theListL.Where(t1 => !AddedTypes.Contains(new KeyValuePair<string, int>(Owner.Name, t1))))
                {
                    ...
                    if (prices.TryGetValue(trueMType, out price))
                    {
                        if (Discont != 0)
                            Price = (int)(price.Item1 - (price.Item1 * ((double)Discont / 100))) < 1 ?
                                price.Item1 : (int)(price.Item1 - (price.Item1 * ((double)Discont / 100)));
                        else
                            Price = price.Item1;
            ...
            stats[StatsType.MerchantDiscount] = Discont;
            stats[StatsType.MerchantMerchandiseType] = trueMType;
            stats[StatsType.MerchantRemainingCount] = mRemaining;
            stats[StatsType.MerchantRemainingMinute] = mTime;
            base.ExportStats(stats);
        ...
        public override void Buy(Player player)
        {
            if (ObjectType == 0x01ca) //Merchant
            {
                if (TryDeduct(player))
                {
                    ...
                    Console.WriteLine("[{3}] {0} have bought {1} for {2}", player.nName, ItemName, truePrice,
                        DateTime.Now);
                }
                else
                {
