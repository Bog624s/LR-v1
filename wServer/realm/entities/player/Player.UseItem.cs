                ...
                switch (eff.Effect)
                {
                    ...
                    case ActivateEffects.StasisBlast:
                    {
                        ...
                        BehaviorBase.AOE(Owner, target, 3, false, enemy =>
                        {
                            ...
                            else if(enemy.HasConditionEffect(ConditionEffects.StunImmume))
                            {
                                pkts.Add(new NotificationPacket
                                {
                                    ObjectId = enemy.Id,
                    ...
                    case ActivateEffects.Create: //this is a portal
                    {
                        ...
                        if (eff.Id == "Wine Cellar Portal") //wine cellar incantation
                        {
                            ...
                            if (opened)
                            {
                                ...
                                w.Timers.Add(new WorldTimer(TimeoutTime*3000,
                    ...
                    case ActivateEffects.UnlockSkin:
                        if (Manager.GameData.Skins.ContainsKey((ushort) eff.SkinType))
                        {
                            ...
                            {
                                Skin = eff.SkinType;
                                PermaSkin = !item.Consumable || item != null;
                                /*if (item != null)
                                {
                                    Effect = (item.FullEffect == "" ? UnusualEffects.Save(item.Effect) : item.FullEffect);
                                }
                                else
                                {
                                    Effect = "";
                    ...
                    case ActivateEffects.Backpack:
                    {
                        ...
                        psr.Character.Backpacks.Add(bps, new short[] {-1, -1, -1, -1, -1, -1, -1, -1});
                        new Database().SaveBackpacks(psr.Character, psr.Account);
                        SendInfo("Added backpack!");
                    }
                        break;
                    ...
                    case ActivateEffects.Gold:
                        {
                            CurrentGold = psr.Account.Stats.Gold = new Database().UpdateCredit(psr.Account, eff.Amount);
                            UpdateCount++;
                        }
                        break;
                    case ActivateEffects.CharSlot:
                        {
                            CurrentCharSlot = psr.Account.Stats.Slot = new Database().UpdateCharSlot(psr.Account, eff.Amount);
                            UpdateCount++;
                        }
                        break;
                    case ActivateEffects.VIP:
                        {
                            CurrentVIP = psr.Account.Stats.VIP = new Database().UpdateVIP(psr.Account, eff.Amount);
                            UpdateCount++;
                        }
                        break;
                    case ActivateEffects.PetGift:
                        {
                            CurrentPetGift = psr.Account.Stats.PetGift = new Database().UpdatePetGift(psr.Account, eff.Amount);
                            UpdateCount++;
                        }
                        break;
