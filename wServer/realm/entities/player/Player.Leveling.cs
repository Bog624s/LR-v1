    ...
    public partial class Player
    {
        private static readonly Dictionary<string, Tuple<int, int, int>> QuestDat =
            new Dictionary<string, Tuple<int, int, int>> //Priority, Min, Max
            {
                ...
                {"Oryx Stone Guardian Left", Tuple.Create(20, 1, 20)},
                {"Oryx Stone Guardian Right", Tuple.Create(20, 1, 20)},
                {"Black Dragon God", Tuple.Create(20, 1, 20)},
                {"Red Dragon God", Tuple.Create(20, 1, 20)},
                {"Blue Dragon God", Tuple.Create(20, 1, 20)},
                {"Green Dragon God", Tuple.Create(20, 1, 20)},
                {"Heir of the Devil", Tuple.Create(20, 1, 20)},
                {"Tomb Defender", Tuple.Create(20, 1, 20)},
                {"Tomb Attacker", Tuple.Create(20, 1, 20)},
                {"Tomb Support", Tuple.Create(20, 1, 20)},
                {"Mysterious Crystal", Tuple.Create(20, 1, 20)},
                {"Avatar of the Forgotten King", Tuple.Create(20, 1, 20)},
                {"Maurth the Succubus Queen", Tuple.Create(20, 1, 20)},
                {"Exp Gift Tower", Tuple.Create(20, 1, 20)},
                {"Undertaker the Great Juggernaut", Tuple.Create(20, 1, 20)},
                {"Eye of the Rock Dragon", Tuple.Create(20, 1, 20)},
                {"McFarvo the Fire God", Tuple.Create(20, 1, 20)},
                {"Mario Bros", Tuple.Create(20, 1, 20)},
                {"Dankins the Boshy Guy", Tuple.Create(20, 1, 20)},
                {"Graw the Overlord", Tuple.Create(20, 1, 20)},
                {"Oryx the Mad God 1", Tuple.Create(20, 1, 20)},
                {"Oryx the Mad God 2", Tuple.Create(20, 1, 20)},
                {"Oryx the Mad God 3", Tuple.Create(20, 1, 20)},
                {"Oryx the Mad God 4", Tuple.Create(20, 1, 20)},
                {"Oryx the Mad God 5", Tuple.Create(20, 1, 20)},
                {"Oryx the Mad God 6", Tuple.Create(20, 1, 20)},
                {"Oryx the Mad God 7", Tuple.Create(20, 1, 20)},
            };
        ...
        private void HandleQuest(RealmTime time)
        {
            if (time.tickCount%500 == 0 || questEntity == null || questEntity.Owner == null)
            {
                ...
                {
                    Owner.Timers.Add(new WorldTimer(10000, (w, t) =>
                    {
        ...
        {
            var newFame = 0;
            if ((Experience < 200*1000) || (Experience > 200*1000)) newFame = Experience/1000;
            //else newFame = 2000 + (Experience - 200*1000)/1000;
            if (newFame != Fame)
            {
                Owner.BroadcastPacket(new NotificationPacket
                {
                    ObjectId = Id,
                    Color = new ARGB(0xff888888),
                    Text = "+1 Fame!"
                }, null);
                Fame = newFame;
                int newGoal;
                ...
                if (newGoal > FameGoal)
                {
                    Owner.BroadcastPacket(new NotificationPacket
                    {
                        ...
                        Text = "You bet! Class Quest Completed!"
                    }, null);
                    Stars = GetStars();
                }
                FameGoal = newGoal;
            ...
            if (Experience - GetLevelExp(Level) >= ExperienceGoal && Level < 20)
            {
                ...
                if (Level == 20)
                    foreach (var i in Owner.Players.Values)
                        i.SendInfo(Name + " achieved level 20! The LoE Team congratulate your effort. If you got questions go to http://loe.ucoz.com/. Visit our website, http://loerealm.com/");
                questEntity = null;
                return true;
            }
            CalculateFame();
        ...
        public bool EnemyKilled(Enemy enemy, int exp, bool killer)
        {
            if (enemy == questEntity)
                ...
                {
                    ObjectId = Id,
                    Color = new ARGB(0x00000000),
                    Text = "Ok, time to next quest..."
                }, null);
            if (exp > 0)
            {
                Experience += exp*2;
