        ...
        protected override bool TickCore(RealmTime time)
        {
            ...
            if (entity != null && dist > targetRadius)
            {
                ...
                if (tx != Host.Self.X || ty != Host.Self.Y)
                {
                    ...
                    var y = Host.Self.Y;
                    var vect = new Vector2(tx, ty) - new Vector2(Host.Self.X, Host.Self.Y);
                    //var vect = new Vector2(Host.Self.X - entity.X, Host.Self.Y - entity.Y);
                    /*vect.Normalize();
                    vect *= (speed/1.5f)*(time.thisTickTimes/1000f);
                    ValidateAndMove(Host.Self.X + vect.X, Host.Self.Y + vect.Y);
                    Host.Self.UpdateCount++;*/
                    ...
                    {
                        var dst = speed/0.5f * (time.thisTickTimes / 1000f);
                        if (vect.Length() > 2)
                            dist = speed/2f * (time.thisTickTimes / 1000f);
                        else if(vect.Length() > 3.5)
                            dist = speed/2f * (time.thisTickTimes / 1000f);
                        else if (vect.Length() > 5)
                            dist = speed/1.5f * (time.thisTickTimes / 1000f);
                        else if (vect.Length() > 6)
                            dist = speed/1.35f * (time.thisTickTimes / 1000f);
                        else if (vect.Length() > 7)
                            dist = speed/1.0f * (time.thisTickTimes / 1000f);
                        else if ((vect.Length() > 10) && (vect.Length() <= 20))
                            dist = speed/0.5f * (time.thisTickTimes / 1000f);
        ...
        protected override bool TickCore(RealmTime time)
        {
            if (Host.Self.PlayerOwner != null)
            {
                if (Host.Self.PlayerOwner.Owner != null)
                {
                    ...
                    if (distance > 20)
                    {
                        Host.Self.PlayerOwner.Move(Host.Self.X, Host.Self.Y);
