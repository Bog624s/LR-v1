...
namespace wServer.logic
{
    ...
    {
        private static _ RedDemon = Behav()
            .Init(0x668, Behaves("Red Demon",
                ...
                new RunBehaviors(
                    Once.Instance(
                        new RunBehaviors(
                            SpawnMinionImmediate.Instance(0x669, 1, 1, 2),
                            SpawnMinionImmediate.Instance(0x66a, 1, 1, 2),
                            SpawnMinionImmediate.Instance(0x66b, 1, 1, 2)
                            )
                        ),
                    new QueuedBehavior(
                        If.Instance(EntityGroupLesserThan.Instance(30, 5, "Demons"),
                            new QueuedBehavior(
                                Rand.Instance(
                    ...
                    Cooldown.Instance(1500, SimpleAttack.Instance(10, 1)),
                    Cooldown.Instance(1000,
                        PredictiveMultiAttack.Instance(10, 5*(float) Math.PI/180, 5, 1, projectileIndex: 0))
                    ),
                loot: new LootBehavior(LootDef.Empty,
                    Tuple.Create(100, new LootDef(0, 2, 0, 8,
                        Tuple.Create(blackbag, (ILoot)new ItemLoot("Demon Horn")),
                        Tuple.Create(whitebag, (ILoot)new ItemLoot("Cyclop's Mace")),
                        ...
                        Tuple.Create(0.6, (ILoot) new StatPotionLoot(StatPotion.Spd)),
                        Tuple.Create(0.6, (ILoot) new StatPotionLoot(StatPotion.Dex))
                        ))
                    ),
